...
import { SEARCH_TYPE_OPTIONS } from "@/lib/media";
    ...
    <li className="flex items-center gap-3 py-3">
      ...
      {result.mediaType === "tv_show" ? (
        <Button
          variant="outline"
          size="sm"
          aria-label={`Seasons of ${result.title}`}
          onClick={onSeasons}
        >
          Seasons
        </Button>
...
export function LogSearch() {
  ...
  const [target, setTarget] = useState<LogTargetInput | null>(null);
  const q = useDebounced(query.trim(), 400);

  const search = useQuery({
    queryKey: ["search", type, q],
    queryFn: () => api.search(type, q),
    enabled: q.length >= 2,
    staleTime: 60_000,
  });
  const seasons = useQuery({
    queryKey: ["seasons", seasonsFor?.externalId],
    queryFn: () => api.seasons(seasonsFor!.externalId),
    enabled: !!seasonsFor,
    staleTime: 60_000,
  });
    ...
    <div className="grid gap-4">
      ...
      {seasonsFor ? (
        ...
        <p className="py-8 text-center text-sm text-muted-foreground">
          Search for a{" "}
          {SEARCH_TYPE_OPTIONS.find((t) => t.value === type)?.label.toLowerCase()}{" "}
          to log it.
        </p>
      ) : search.isLoading ? (
